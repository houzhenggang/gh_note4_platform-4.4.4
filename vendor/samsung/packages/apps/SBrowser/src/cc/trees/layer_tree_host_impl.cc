...
//Fling velocity threshold is used to detect the condition whether the scroll
//delta need to apply to content also after applying that to topcontrols.
const float kFlingThresholdVelocity = 400.0f;
#endif
...
LayerTreeHostImpl::LayerTreeHostImpl(
    ...
    : client_(client),
      ...
      id_(id),
      viewport_changed_(false)
#if defined(S_LOW_LES_ON_FASTSCROLL)
      ,fast_fling_(false)
#endif
// SAMSUNG CHANGE : Check for scrolling or fling
#if defined(S_NATIVE_SUPPORT)
      ,scroll_gesture_active_(false)
#endif
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)   	  
      ,onFlingStarted_(false)
#endif
#if defined(SBROWSER_HIDE_URLBAR_EOP)	  
      ,is_end_of_page_(false)
      ,prev_is_end_of_page_(false)
#endif
#if defined(S_SCROLLBAR_LESS_UPDATE)
      ,schedule_to_hide_scrollbar(false)
      ,scrollbar_request_counter_(0)
#endif
	  {
  ...
  SetDebugState(settings.initial_debug_state);
#if defined(S_DEFER_COMMIT_ON_GESTURE)
  defer_commit_ = false;
#endif
#if defined(S_PLM_P140823_03525)
  scroll_type_ = TOUCH;
#endif
...
void LayerTreeHostImpl::StartPageScaleAnimation(
  ...
  scoped_ptr<TimingFunction> timing_function =
      CubicBezierTimingFunction::Create(0, 0, .3, .9).PassAs<TimingFunction>();
...
DrawSwapReadbackResult::DrawResult LayerTreeHostImpl::CalculateRenderPasses(
  ...
  if (root_surface_has_contributing_layers &&
      ...
      active_tree_->LayersWithCopyOutputRequest().empty() && !page_scale_animation_active()) {
    TRACE_EVENT0("cc",
...
// SAMSUNG CHANGE : Check for scrolling or fling
  ...
  if (num_missing_tiles) {
    HISTOGRAM_COUNTS_100("LayerTreeHostImpl.NumMissingTiles", num_missing_tiles);
  }
...
void LayerTreeHostImpl::DidDrawAllLayers(const FrameData& frame) {
  ...
  if (viewport_changed_) {
    device_viewport_size_ = new_viewport_size_;
    viewport_changed_ = false;
    SetFullRootLayerDamage();
  }
}
...
gfx::SizeF LayerTreeHostImpl::ComputeInnerViewportContainerSize() const {
  ...
  return gfx::SizeF(dip_size.width(),
                    dip_size.height() - overdraw_bottom_height_);
#else					
  float top_offset =
...
void LayerTreeHostImpl::SetVisible(bool visible) {
  ...
  if (!visible_)
  {
    ManageTiles();
#if defined(S_NATIVE_SUPPORT)
	if(scroll_gesture_active_ == true)
    	ScrollEnd();
#endif
  }
  if (!renderer_)
...
#if defined(SBROWSER_MAX_RASTER_BYTES_MODIFICATION)
  tile_manager_->setDefaultTileSize(settings_.default_tile_size.width());
#endif
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)
  //Notifies app that the renderer intialization is done
  if(output_surface_)
  	output_surface_->OnRendererInitializeComplete();
#endif
  return true;
}
...
#if defined(OS_ANDROID)
  // FIXME(heejin): This is a temporary fix for the black patches
  // in LowEndDevices that appear while rotating.
  // After resizing, regions out of the viewport seem to remain dirty,
  // so I squeezed in a frame with the viewport set to full screen.
  if (base::android::SysUtils::IsLowEndDevice() &&
      !pending_tree_) {
    device_viewport_size_ = gfx::Size(
        std::max(device_viewport_size.width(),
                 device_viewport_size_.width()),
        std::max(device_viewport_size.height(),
                 device_viewport_size_.height()));
    new_viewport_size_ = device_viewport_size;
    viewport_changed_ = true;
  } else
#endif
...
void LayerTreeHostImpl::DidChangeTopControlsPosition() {
  ...
  if (top_controls_manager_ 
  	&& (top_controls_manager_->content_top_offset() == 0.f || top_controls_manager_->content_top_offset() == settings_.top_controls_height)){
  	client_->SetContentTopOffsetToMainThread(top_controls_manager_->content_top_offset());
  }
  #endif
}
...
#if defined(S_PLM_P140916_05275)
...
#if defined(S_LOW_LES_ON_FASTSCROLL)
  accumulated_fling_scroll_ = gfx::Vector2dF();
  fast_fling_ = false;
#endif
// SAMSUNG CHANGE : Check for scrolling or fling
#if defined(S_NATIVE_SUPPORT)
...
bool LayerTreeHostImpl::ScrollBy(gfx::Point viewport_point,
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)   	   
  bool force_consume_by_top_controls =
      top_controls_manager_ &&
      CurrentlyScrollingLayer() != InnerViewportScrollLayer() &&
            InnerViewportScrollLayer()->bounds().width() <= CurrentlyScrollingLayer()->bounds().width() &&
            (InnerViewportScrollLayer()->bounds().height() / 2) <= CurrentlyScrollingLayer()->bounds().height();
  #if defined(S_PLM_P140823_03525)
  if(scroll_type_ == HOVER_SCROLLING && (force_consume_by_top_controls || consume_by_top_controls)){
	force_consume_by_top_controls = false;
	consume_by_top_controls = false;
  }
  #endif
#endif
  ...
  for (LayerImpl* layer_impl = CurrentlyScrollingLayer();
    ...
    if (!has_scrollBar &&
        (layer_impl->HasScrollbar(VERTICAL) ||
         layer_impl->HasScrollbar(HORIZONTAL))) {
      has_scrollBar = true;
    }
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID) 		
        if(onFlingStarted_ == true 
		   && std::abs(current_fling_velocity_.y()) > kFlingThresholdVelocity 
		   && std::abs(pending_delta.y()) < 0.001 
		   && std::abs(applied_delta.y())>0 )
        {
          pending_delta.set_y( pending_delta.y() + applied_delta.y() );
        }
#endif		
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)  	
    else if (force_consume_by_top_controls)
...
#if defined(S_LOW_LES_ON_FASTSCROLL)
    accumulated_fling_scroll_ += gfx::Vector2dF(applied_delta.x(),applied_delta.y());
	bool did_move_fast_scroll_x = std::abs(accumulated_fling_scroll_.x()) > device_viewport_size_.width();
	bool did_move_fast_scroll_y = std::abs(accumulated_fling_scroll_.y()) > device_viewport_size_.height();

    if(did_move_fast_scroll_x
      ||did_move_fast_scroll_y) {
      fast_fling_ = true;
    } else {
      fast_fling_ = false;
    }
...
#if defined(SBROWSER_HIDE_URLBAR_EOP) 
    if( layer_impl == InnerViewportScrollLayer()
	&& scroll_delta.y() > 0 	// checking scroll direction
	&& (pending_delta.y() - applied_delta.y()) > move_threshold ) {
      is_end_of_page_ = true;
    } else if( top_controls_manager_->controls_top_offset() == -settings_.top_controls_height 
	      || top_controls_manager_->controls_top_offset() == 0 ){
      is_end_of_page_ = false;
    }
#endif
...
#if defined(S_SCROLLBAR_LESS_UPDATE)
    ScrollbarAnimationController* scrollbar_controller = layer_impl->scrollbar_animation_controller();
    if (scrollbar_controller) {
      scrollbar_controller->SetScrollGestureInProgress(true);
    }
#endif
...
// Below code will return with out setting did_overscroll parameter if no scrollbars are present.
//Commenting the code for PLM: P140722-06648. 
 /* if (!has_scrollBar)
    return did_scroll;*/

#if defined(S_MODIFY_OVERSCROLL_CONDITION)
  gfx::Vector2dF overscroll_delta = unused_root_delta;
  if(InnerViewportScrollLayer()->MaxScrollOffset().x() == 0){
	  overscroll_delta.set_x(0);
  }
  if(InnerViewportScrollLayer()->MaxScrollOffset().y() == 0){
	  overscroll_delta.set_y(0);
  }
  bool did_overscroll = !gfx::ToRoundedVector2d(overscroll_delta).IsZero();
#else
  bool did_overscroll = !gfx::ToRoundedVector2d(scroll_delta).IsZero();
#endif
...
#if defined(SBROWSER_HIDE_URLBAR_EOP)
  if( prev_is_end_of_page_ != is_end_of_page_ ) {
    if(output_surface_)
      output_surface_->UpdateEndOfPageState(is_end_of_page_);    
    prev_is_end_of_page_ = is_end_of_page_;
  }
  top_controls_manager_->set_end_of_page(is_end_of_page_);
#endif
  return did_scroll;
}
...
// SAMSUNG CHANGE : Check for scrolling or fling
  ...
  ClearCurrentlyScrollingLayer();
#if defined(S_SCROLLBAR_LESS_UPDATE)
  schedule_to_hide_scrollbar = true;
  SetScrollbarRequestCount(true);
  base::TimeDelta fadeout_delay = base::TimeDelta::FromMilliseconds(1000);
  client_->RequestScrollbarAnimationOnImplThread(fadeout_delay);
#else
...
#if !defined(SBROWSER_HIDE_URLBAR_HYBRID)   
  // Don't need to call ScrollViewPortBy , this will invoke content scroll unnessarily which we not required.
...
#if defined(S_SCROLLBAR_LESS_UPDATE)
  if(scrollbar_controller)
      scrollbar_controller->SetScrollGestureInProgress(false);
#endif
...
#if defined(S_SCROLLBAR_LESS_UPDATE)
void LayerTreeHostImpl::SetScrollbarRequestCount(bool increase_count ) {
  if(increase_count)
    scrollbar_request_counter_++;
  else
    scrollbar_request_counter_--;
}

int LayerTreeHostImpl::GetScrollbarRequestCount() {
  return scrollbar_request_counter_;
}
#endif
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)
void LayerTreeHostImpl::SetTopControlsHeight(int top_controls_height) {

  if( top_controls_height > 0){

      if( top_controls_manager_ )
          if( top_controls_manager_->controls_top_offset() == -settings_.top_controls_height ) {
			top_controls_manager_->set_controls_top_offset( -top_controls_height );
          }

      settings_.top_controls_height = top_controls_height;
      if (!top_controls_manager_) {

          top_controls_manager_ =
			TopControlsManager::Create(this,
			    settings_.top_controls_height,
			    settings_.top_controls_show_threshold,
			    settings_.top_controls_hide_threshold);
      }
      top_controls_manager_->setTopControlsHeight(top_controls_height,
			    settings_.top_controls_show_threshold,
			    settings_.top_controls_hide_threshold);
  }
  else
  {
      settings_.top_controls_height = 0;      
      if (top_controls_manager_) {
          top_controls_manager_->setTopControlsHeight(0,
			    settings_.top_controls_show_threshold,
			    settings_.top_controls_hide_threshold);
          top_controls_manager_->set_controls_top_offset(0);
      }
  }
}
#endif
...
void LayerTreeHostImpl::SetScrollType(int type) {
    scroll_type_ = type;
}
#endif

#if defined(S_DEFER_COMMIT_ON_GESTURE)
void LayerTreeHostImpl::DeferCommitsOnGesture(bool defer_commit) {
    if(defer_commit_ == defer_commit)
        return;
    TRACE_EVENT1("cc", "LayerTreeHostImpl::DeferCommitsOnGesture","defer_commit",defer_commit);
    if(client_){
        if(visible_){
            client_->DeferCommitsOnImplThread(defer_commit);
            defer_commit_ = defer_commit;
        }else{
            client_->DeferCommitsOnImplThread(false);
            defer_commit_ = false;
        }
    }
}

void LayerTreeHostImpl::SetDeferCommitOnImpl(bool defer) {
    DeferCommitsOnGesture(defer);
}
#endif
}  // namespace cc
