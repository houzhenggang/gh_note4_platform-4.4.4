...
TEST_F(LayerTreeHostImplTest, ScrollbarLinearFadeScheduling) {
  ...
  EXPECT_GT(base::TimeDelta::FromMilliseconds(19),
            requested_scrollbar_animation_delay_);
#else
  EXPECT_LT(base::TimeDelta::FromMilliseconds(19),
            requested_scrollbar_animation_delay_);
...
class LayerTreeHostImplTopControlsTest : public LayerTreeHostImplTest {
 public:
  ...
  void SetupTopControlsAndScrollLayer() {
    ...
    LOG(INFO)<<"Skipping test EXPECT_EQ(clip_size_, root_clip_ptr->bounds()) due to topControls design change";
#else
...
TEST_F(LayerTreeHostImplTopControlsTest, ScrollTopControlsByFractionalAmount) {
  ...
  host_impl_->ScrollEnd();
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)  
	LOG(INFO)<<"Skipping test  EXPECT_EQ( top_controls_scroll_delta, inner_viewport_scroll_layer->FixedContainerSizeDelta()) due to topControls design change";
#else  	
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)  
	LOG(INFO)<<"Skipping test  EXPECT_LT((expected_container_size_delta - inner_viewport_scroll_layer->FixedContainerSizeDelta()).LengthSquared(),tolerance); due to topControls design change";
#else 
  gfx::Vector2dF expected_container_size_delta =
