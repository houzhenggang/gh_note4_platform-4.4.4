...
#if defined(S_PLM_P140806_02267)
...
gfx::Vector2dF TopControlsManager::ScrollBy(
  ...
  if (permitted_state_ == SHOWN && pending_delta.y() > 0)
    ...
    return pending_delta;
#if defined(SBROWSER_HIDE_URLBAR_EOP)
  if( !(is_end_of_page_ && controls_top_offset_ == -top_controls_height_))
#endif
...
void TopControlsManager::setTopControlsHeight(float top_controls_height,
  ...
  top_controls_height_ = top_controls_height;
  top_controls_show_height_ = 
          top_controls_height_ * top_controls_hide_threshold;
  top_controls_hide_height_ =
          top_controls_height_ * (1.f - top_controls_show_threshold);
}
#endif
