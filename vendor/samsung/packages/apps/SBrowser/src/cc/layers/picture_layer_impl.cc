...
const float kAlmostMaxScaleRatioDuringPinch = 0.9999f * kMaxScaleRatioDuringPinch;
...
PictureLayerImpl::PictureLayerImpl(LayerTreeImpl* tree_impl, int id)
    : LayerImpl(tree_impl, id),
      ...
      ,should_use_low_res_tiling_on_fling_(false)
#endif
#if !defined(SBROWSER_GPU_RASTERIZATION_ENABLE)
...
scoped_refptr<Tile> PictureLayerImpl::CreateTile(PictureLayerTiling* tiling,
  ...
  layer_tree_impl()->SetDeferCommitOnImpl(false);
#endif
...
bool PictureLayerImpl::MarkVisibleTilesAsRequired(
  ...
  //FIXME: high res tiling should be existed.
  if (!tiling)
    return false;
...
#if defined(S_LOW_LES_ON_FASTSCROLL)
  if ( layer_tree_impl()->IsFastFling() )
  {
  	change_target_tiling = true;
	should_use_low_res_tiling_on_fling_ = true;
  }
  else 
  	should_use_low_res_tiling_on_fling_ = false;
#endif
...
float PictureLayerImpl::SnappedContentsScale(float scale) {
  ...
  float const max_acceptable_contents_scale = ideal_contents_scale_;
  float const min_acceptable_contents_scale = ideal_contents_scale_ / kAlmostMaxScaleRatioDuringPinch;
  ...
  for (size_t i = 0; i < tilings_->num_tilings(); ++i) {
    ...
    if (tiling_contents_scale > max_acceptable_contents_scale ||
        tiling_contents_scale < min_acceptable_contents_scale)
      continue;
...
void PictureLayerImpl::RecalculateRasterScales(
  ...
  bool is_pinching = layer_tree_impl()->PinchGestureActive();
  bool is_scaling  = layer_tree_impl()->PageScaleAnimationActive();
  if (!(is_pinching || is_scaling) || raster_contents_scale_ == 0.f) {
    ...
    // - When zooming in, jump to ideal.
    bool zooming_out = raster_page_scale_ > ideal_page_scale_;
    float desired_contents_scale =
        zooming_out ? ideal_contents_scale_ / kAlmostMaxScaleRatioDuringPinch
                    : ideal_contents_scale_;
    raster_contents_scale_ = SnappedContentsScale(desired_contents_scale);
    // Avoid fatal rounding errors.
    raster_page_scale_ = std::min(raster_contents_scale_ / raster_device_scale_, ideal_page_scale_);
  }
