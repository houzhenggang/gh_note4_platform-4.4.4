    ...
    case GLTexture: {
      ...
      if (gr_context) {
        ...
        surface = skia::AdoptRef(
            SkSurface::NewRenderTargetDirect(gr_context, gr_texture->asRenderTarget()));
#else
        surface = skia::AdoptRef(
...
#if defined (USE_SYSTEM_SKIA)
      SkImageInfo image_info = {resource()->size.width(), resource()->size.height(),kBGRA_8888_SkColorType, kPremul_SkAlphaType};
#else
...
#if defined(SBROWSER_GPU_RASTERIZATION_ENABLE)
    SkColorType buffer_colorType = ResourceFormatToSkColorType(resource()->format);
...
void ResourceProvider::DeleteResource(ResourceId id) {
  ...
  if (it == resources_.end()) {
    LOG(ERROR)<<"Can not find the resource by id";
    return;
  }
