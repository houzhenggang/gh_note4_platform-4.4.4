...
void FlingAnimatorImpl::StartFling(const gfx::PointF& velocity)
{
  ...
  float dpi_scale = gfx::Screen::GetNativeScreen()->GetPrimaryDisplay()
      .device_scale_factor();
// SAMSUNG CHANGE : Change fling velocity
#if defined(S_NATIVE_SUPPORT)
  JNI_OverScroller::Java_OverScroller_flingV_I_I_I_I_I_I_I_I(
      env, java_scroller_.obj(), 0, 0,
      static_cast<int>(velocity.x() * dpi_scale*0.88f),//0.66f),
      static_cast<int>(velocity.y() * dpi_scale*0.88f),//0.66f),
...
// SAMSUNG CHANGE : Change fling velocity
#if defined(S_NATIVE_SUPPORT)
      velocity = GetCurrentVelocity()*0.66f;
#endif
