...
void UpdateThumbnailWithURL(const ThumbnailingContext& context,
                            const GURL& gurl,
                            const SkBitmap& thumbnail) {
  gfx::Image image = gfx::Image::CreateFrom1xBitmap(thumbnail);
  context.service->SetPageThumbnailWithURL(context, gurl, image);
  VLOG(0) << "Thumbnail taken for " << gurl.spec() << ": "
        << context.score.ToString();
  VLOG(0) << " Thumbnail context url = " << context.url;
}
#endif
...
#if defined(S_NATIVE_SUPPORT)
#if defined(ENABLE_MOSTVISITED)
void ThumbnailTabHelper::UpdateThumbnailWithOriginalURL(
  ...
  if (thumbnail_service == NULL  ||
      !thumbnail_service->ShouldAcquirePage(url)) {
    LOG(ERROR) << " Page is not suitable for adding again.";
    return;
  }

  scoped_refptr<thumbnails::ThumbnailingAlgorithm> algorithm(
      thumbnail_service->GetThumbnailingAlgorithm());
  algorithm->ProcessBitmapWithURL(context, base::Bind(&UpdateThumbnailWithURL),
      gurl, thumbnail);
}
