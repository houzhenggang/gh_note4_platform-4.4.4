...
public abstract class TabBase implements NavigationClient {
    ...
    private static final String TAG = "TabBase";
    ...
    protected class TabBaseWebContentsObserverAndroid extends WebContentsObserverAndroid {
    ...
    /*
     *return native web contents pointer
     */
    protected long getWebContentsPtr() {
        if (mNativeTabAndroid == 0) return 0;
        return nativeGetWebContentsPtr(mNativeTabAndroid);
    }
    // SBROWSER_MULTIINSTANCE_TAB_DRAG_N_DROP>>
    ...
    protected void initContentView(long nativeWebContents) {
        ...
        mWebContentsObserver = createWebContentsObserver(mContentViewCore);
        mVoiceSearchTabHelper = createVoiceHelper(mContentViewCore);
        ...
        if (mInfoBarContainer == null) {
            ...
            // initialized.
            mInfoBarContainer = createInfoBarContainer(nativeWebContents);
        } else {
    ...
    public Bitmap getFavicon() {
        if(mNativeTabAndroid == 0 || mContentViewCore == null)
            return null;
        String url = mContentViewCore.getUrl();
        if(url == null) return null;
        if(mFavicon == null || !url.equals(mFaviconUrl)) {
    ...
    protected final void destroyContentView(boolean deleteNativeWebContents) {
        ...
        Log.i(TAG, "destroyContentView calling destroyContentViewInternal " + mContentView);
        ...
        if (mContentViewCore != null) {
            Log.i(TAG, "destroyContentView calling mContentViewCore.destroy " + mContentViewCore);
        ...
        assert mNativeTabAndroid != 0;
        Log.i(TAG, "destroyContentView calling nativeDestroyWebContents");
     ...
     * @return An instance of a {@link TabBaseWebContentsObserverAndroid}.
     */
    protected TabBaseWebContentsObserverAndroid createWebContentsObserver(
            ContentViewCore contentViewCore) {
        return new TabBaseWebContentsObserverAndroid(contentViewCore);
    }

    protected VoiceSearchTabHelper createVoiceHelper(
            ContentViewCore contentViewCore) {
        return new VoiceSearchTabHelper(contentViewCore);
    }


    /**
     * A helper method to allow subclasses to build their own InfoBar containers.
     * @param nativeWebContents
     * @return An instance of a {@link InfobarContainer}.
     */
    protected InfoBarContainer createInfoBarContainer(long nativeWebContents) {
        return new  InfoBarContainer(
    ...
    @CalledByNative
    protected void onPopupBlockStateChanged() {
        for (TabObserver observer : mObservers)
            observer.onPopupBlockStateChanged(this);
    }
    ...
    /**
     * Called when page open a Authentication Dialog.
     */
    @CalledByNative
    protected void OnReceivedHttpAuthRequest(ChromeHttpAuthHandler authHandler, String host, String realm) {
    ...
    @CalledByNative
    protected void swapWebContents(
    ...
    /**
     * Set custom ContentView used for building up
     * independent modules that access ContentView
     * methods.
     * @param customContentView ContentView object
     */
    public void setContentView(ContentView customContentView) {
        mContentView = customContentView;
    }
    ...
    // SBROWSER_MULTIINSTANCE_TAB_DRAG_N_DROP<<
    private native long nativeGetWebContentsPtr(long nativeTabAndroid);
    // SBROWSER_MULTIINSTANCE_TAB_DRAG_N_DROP>>
