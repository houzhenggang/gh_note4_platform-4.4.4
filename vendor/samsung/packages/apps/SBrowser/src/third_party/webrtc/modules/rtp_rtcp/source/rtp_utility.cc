...
*/
  ...
  if (CurrentFrameH264Hdr.nal_unit_payload_type == NAL_FU_FORMAT) {
    if (CurrentFrameH264Hdr.u_nalu.fu_nal_unit.s_bit == true) {
      dataBuffer = (uint8_t *)malloc(sizeof(uint8_t) * (dataLength + 1));
      memset(dataBuffer, 0, dataLength + 1);

      /* reconstruct NAL header */
      dataBuffer[0] = (dataPtr[0] & 0xe0) | (dataPtr[1] & 0x1f);
      memcpy (dataBuffer + 1, dataPtr + offset, dataLength - offset);
      parsedPacket.info.H264.data = dataBuffer;
