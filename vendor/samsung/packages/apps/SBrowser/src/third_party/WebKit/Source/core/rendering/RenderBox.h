...
public:
    ...
    {
        if (!isTableCell() && LIKELY(style()->paddingLeft() == style()->paddingRight())) {
            // Fast path if same padding on left / right.
            LayoutUnit padding = computedCSSPaddingLeft();
            ASSERT(padding == paddingLeft());
            ASSERT(padding == paddingRight());
            return clientWidth() - 2 * padding;
        }
        return clientWidth() - paddingLeft() - paddingRight();
    }
    LayoutUnit contentHeight() const
    {
        if (!isTableCell() && LIKELY(style()->paddingTop() == style()->paddingBottom())) {
            // Fast path if same padding on top / bottom.
            LayoutUnit padding = computedCSSPaddingTop();
            ASSERT(padding == paddingTop());
            ASSERT(padding == paddingBottom());
            return clientHeight() - 2 * padding;
        }
    ...
    void scrollRectToVisible(const LayoutRect&, const ScrollAlignment& alignX, const ScrollAlignment& alignY,bool value=false);
#else
...
#if defined(S_TEXT_SELECTION_BUGFIXES)
    , bool bTextSelection=false
#endif
) OVERRIDE;
