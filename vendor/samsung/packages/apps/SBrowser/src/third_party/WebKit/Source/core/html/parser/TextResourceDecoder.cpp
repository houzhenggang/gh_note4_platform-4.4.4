...
#if defined(S_PLM_P140819_01865)
...
String TextResourceDecoder::decode(const char* data, size_t len)
{
	...
	if ((m_contentType == HTMLContent || m_contentType == XMLContent) && !m_checkedForHeadCharset)
    ...
    if (m_buffer.isEmpty())
        return m_codec->decode(data + lengthOfBOM, len - lengthOfBOM, false, m_contentType == XMLContent, m_sawError);
    ...
    String result = m_codec->decode(m_buffer.data() + lengthOfBOM, m_buffer.size() - lengthOfBOM, false, m_contentType == XMLContent && !m_useLenientXMLDecoding, m_sawError);
    m_buffer.clear();
    return result;
}
#else
...
String TextResourceDecoder::flush()
{
    ...
    if (m_buffer.size() && shouldAutoDetect()
        && ((!m_checkedForHeadCharset && (m_contentType == HTMLContent || m_contentType == XMLContent)) || (!m_checkedForCSSCharset && (m_contentType == CSSContent)))) {
         WTF::TextEncoding detectedEncoding;
