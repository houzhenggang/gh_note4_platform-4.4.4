...
FrameView::FrameView(Frame* frame)
    ...
    , m_scrollBehavior(ScrollBehaviorInstant)
    #endif
    #if defined(S_IME_SCROLL_EVENT)
    , m_contentTopOffset(0.f)
    #endif
...
void FrameView::setScrollPosition(const IntPoint& scrollPoint, ScrollBehavior scrollBehavior)
{
    ...
    //Reset the ScrollBehavior
    setScrollBehavior(ScrollBehaviorInstant);
}

void FrameView::setScrollBehavior(ScrollBehavior scroll_behavior){
	m_scrollBehavior = scroll_behavior;
}
#else
...
void FrameView::scrollPositionChanged()
{
    ...
    if (RenderView* renderView = document->renderView()) {
        renderView->repaintImageAnimationsIfNeeded();
        if (renderView->usesCompositing()) {
            // https://code.google.com/p/chromium/issues/detail?id=343767
            DisableCompositingQueryAsserts disabler;
...
#if defined(S_IME_SCROLL_EVENT)
void FrameView::setContentTopOffset(float contentTopOffset){
    m_contentTopOffset = contentTopOffset;
}

float FrameView::getContentTopOffset(){
    return m_contentTopOffset;
}
#endif
...
void FrameView::paintContents(GraphicsContext* p, const IntRect& rect)
{
    ...
    // Incase of WML mimetype based HTMLViewSourceDocument, then avoid painting
    // page source and application level, WML shall be translated into HTML and loaded.
    if (document->isViewSource() && document->isWMLDocument()) {
...
#if defined (SBROWSER_SOFTBITMAP_IMPL)
        m_softBitmapValidCapture = false;
#endif
        return;
    }
    ...
    m_isPainting = true;
#if defined(SBROWSER_PRINT_PAINT_LOG)
    if(frame().page() && frame().page()->shouldPrintPaintLog()){
	LOG(INFO) << "FrameView::paintContents";
	frame().page()->setShouldPrintPaintLog(false);
    }
#endif
