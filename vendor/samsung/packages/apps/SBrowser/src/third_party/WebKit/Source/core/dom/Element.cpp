...
void Element::scrollIntoViewIfNeeded(bool centerIfNeeded)
{
    ...
    if(element  && element->hasTagName(selectTag)){
        if (centerIfNeeded)
            renderer()->scrollRectToVisible(bounds, ScrollAlignment::alignCenterIfNeeded, ScrollAlignment::alignCenterIfNeeded,true);
        else
            renderer()->scrollRectToVisible(bounds, ScrollAlignment::alignToEdgeIfNeeded, ScrollAlignment::alignToEdgeIfNeeded,true);
    }else{
        if (centerIfNeeded)
...
void Element::scrollIntoViewCenter(bool centerAlways)
{
    document().updateLayoutIgnorePendingStylesheets();
    ...
    LayoutRect bounds = boundingBox();
    if (centerAlways)
        renderer->scrollRectToVisible(bounds, ScrollAlignment::alignCenterAlways, ScrollAlignment::alignCenterAlways);
    else
        renderer->scrollRectToVisible(bounds, ScrollAlignment::alignCenterIfNeeded, ScrollAlignment::alignCenterIfNeeded);
}
...
ALWAYS_INLINE void Element::setAttributeInternal(size_t index, const QualifiedName& name, const AtomicString& newValue, SynchronizationOfLazyAttribute inSynchronizationOfLazyAttribute)
{
    ...
    bool sameValue = newValue == existingAttribute->value();
    if (!sameValue) {
    ...
    if (!inSynchronizationOfLazyAttribute)
        didModifyAttribute(existingAttributeName, newValue, sameValue);
}
...
void Element::attributeChanged(const QualifiedName& name, const AtomicString& newValue, bool sameValue, AttributeModificationReason reason)
{
...
inline void Element::attributeChangedFromParserOrByCloning(const QualifiedName& name, const AtomicString& newValue, AttributeModificationReason reason)
{
    ...
    attributeChanged(name, newValue, false, reason);
}
...
void Element::recalcStyle(StyleRecalcChange change, Text* nextTextSibling, bool scoped)
{
    ASSERT(document().inStyleRecalc());
    ASSERT(scoped || !parentOrShadowHostNode()->needsStyleRecalc());

    if (!scoped && hasCustomStyleCallbacks())
    ...
    if (change >= Inherit || needsStyleRecalc()) {
        if (hasRareData()) {
            ...
            if (!scoped && change >= Inherit) {
                if (ActiveAnimations* activeAnimations = data->activeAnimations())
                    activeAnimations->setAnimationStyleChange(false);
            }
        }
        if (parentRenderStyle())
            change = recalcOwnStyle(change, scoped);
        if (scoped)
            setStyleChangeState(HasScopedRecalcStyle);
    ...
    clearStyleChangeState();
}

StyleRecalcChange Element::recalcOwnStyle(StyleRecalcChange change, bool scoped)
{
    ASSERT(document().inStyleRecalc());
    ASSERT(scoped || !parentOrShadowHostNode()->needsStyleRecalc());
    ...
    RefPtr<RenderStyle> newStyle = !scoped ? styleForRenderer() : document().ensureStyleResolver().styleForElement(this, 0, DisallowStyleSharing);
...
void Element::checkForChildrenAdjacentRuleChanges()
{
    ...
    for (Node* child = firstChild(); child; child = child->nextSibling()) {
        ...
        if (forceCheckOfNextElementCount || forceCheckOfAnyElementSibling)
            element->setNeedsStyleRecalc(SubtreeStyleChange, StyleChangeFromCSS, AffectedByDirectAdjacentAndPositionalRules);
...
void Element::checkForSiblingStyleChanges(bool finishedParsingCallback, Node* beforeChange, Node* afterChange, int childCountDelta)
{
    ...
    if ((childrenAffectedByForwardPositionalRules() && afterChange) || (childrenAffectedByBackwardPositionalRules() && beforeChange)) {
        setNeedsStyleRecalc(SubtreeStyleChange, StyleChangeFromCSS, ChildrenAffectedByBackForwardPositionalRules);
        return;
    }
...
void Element::didModifyAttribute(const QualifiedName& name, const AtomicString& value, bool sameValue)
{
    attributeChanged(name, value, sameValue);
    InspectorInstrumentation::didModifyDOMAttr(this, name.localName(), value);
