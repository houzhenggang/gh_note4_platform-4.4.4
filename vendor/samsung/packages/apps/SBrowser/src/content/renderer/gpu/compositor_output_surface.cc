    ...
    base::TaskRunner* target_task_runner)
{
  uint32 messages_to_filter[] = {
    ...
    ,ViewMsg_SetTopControlsHeight::ID
#endif
#if defined(S_SET_SCROLL_TYPE)
    ,ViewMsg_SetScrollType::ID
#endif
  };
...
#endif
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)
    IPC_MESSAGE_HANDLER(ViewMsg_SetTopControlsHeight, OnSetTopControlsHeight);
#endif
#if defined(S_SET_SCROLL_TYPE)
    IPC_MESSAGE_HANDLER(ViewMsg_SetScrollType, OnSetScrollType);
#endif
  IPC_END_MESSAGE_MAP()
}
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)
void CompositorOutputSurface::OnRendererInitializeComplete()
{
  Send(new ViewHostMsg_OnRendererInitializeComplete(routing_id_));
}

void CompositorOutputSurface::OnSetTopControlsHeight(int top_controls_height) {
  DCHECK(CalledOnValidThread());
...
#if defined(S_SET_SCROLL_TYPE)
void CompositorOutputSurface::OnSetScrollType(int type) {
  DCHECK(CalledOnValidThread());
  SetScrollType(type);
}
#endif
...
void CompositorOutputSurface::UpdateEndOfPageState( bool eop_state ) {
  Send(new ViewHostMsg_OnUpdateEndOfPageState(routing_id_,eop_state));
}
#endif
