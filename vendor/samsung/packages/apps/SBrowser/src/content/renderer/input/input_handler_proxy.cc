...
InputHandlerProxy::EventDisposition InputHandlerProxy::HandleInputEvent(
  ...
  if (event.type == WebInputEvent::MouseWheel) {
...
#endif
    LOG(INFO) << "[SBRCHECK_LU] InputHandlerProxy::HandleInputEvent  WebInputEvent::GestureScrollBegin enter";
    ...
    DCHECK(expect_scroll_update_end_);
#endif
	//LOG(INFO) << "[SBRCHECK_LU] InputHandlerProxy::HandleInputEvent  WebInputEvent::GestureScrollUpdate enter";
	...
	//LOG(INFO) << "[SBRCHECK_LU] InputHandlerProxy::HandleInputEvent  WebInputEvent::GestureScrollEnd enter";
    ...
    LOG(INFO) << "[SBRCHECK_LU] InputHandlerProxy::HandleInputEvent  WebInputEvent::TouchStart enter";
...
void InputHandlerProxy::scrollBy(const WebFloatSize& increment) {
  ...
  static bool disable_block_fling_on_edge =
    CommandLine::ForCurrentProcess()->HasSwitch(
      cc::switches::kDisableSbrowserBlockFlingOnEdge);
  if (!disable_block_fling_on_edge)
    block_fling_on_edge_ = !did_scroll;
