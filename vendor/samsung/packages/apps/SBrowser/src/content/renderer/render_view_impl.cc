...
void RenderViewImpl::OnNavigate(const FrameMsg_Navigate_Params& params) {
  LOG(INFO) << "[SBRCHECK_LU] RenderViewImpl::OnNavigate: Received IPC [LoadURL] [RenderProcess] [RoutingID=" << GetRoutingID() << "]";
...
#if defined(S_PLM_P141007_00217)
    //Add a check for WebDataSource ds to avoid crash.
    if (ds){
#endif
    DocumentState* document_state = DocumentState::FromDataSource(ds);
    InternalDocumentStateData* data =
