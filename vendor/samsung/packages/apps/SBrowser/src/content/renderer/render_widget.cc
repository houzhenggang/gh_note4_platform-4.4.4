...
RenderWidget::RenderWidget(blink::WebPopupType popup_type,
    ...
    : routing_id_(MSG_ROUTING_NONE),
      ...
      seen_processed_touch_move_(false)
{
  if (!swapped_out)
...
bool RenderWidget::OnMessageReceived(const IPC::Message& message) {
  ...
  IPC_BEGIN_MESSAGE_MAP(RenderWidget, message)
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_DefersLoading, OnDefersLoading)
#endif
    ...
    IPC_MESSAGE_HANDLER(ViewMsg_UpdateTextInputState, UpdateTextInputStateForReset)
#endif
...
void RenderWidget::OnResize(const ViewMsg_Resize_Params& params) {
  ...
  LOG(INFO)<<"RenderWidget::OnResize size=" << params.new_size.width() << "," << params.new_size.height();
#endif
...
#if defined(SBROWSER_PRINT_PAINT_LOG)  
   LOG(INFO)<<"RenderWidget::OnWasShown() calling scheduleComposite";
#endif
    if (compositor_)
      compositor_->SetNeedsForcedRedraw();
    scheduleComposite();
...
void RenderWidget::OnHandleInputEvent(const blink::WebInputEvent* input_event,
  ...
  if(input_event->type == WebInputEvent::RawKeyDown ){
	...
	if( key_event.nativeKeyCode == AKEYCODE_ENTER){
  		RenderViewImpl* render_view = static_cast<RenderViewImpl*> (this);
		render_view->OnEnterKeyOnFocused();
	 }
  }
#endif
...
// SAMSUNG CHANGE : Patch for scrolling rate limited issue
#if defined(S_NATIVE_SUPPORT)
  if (input_event->type == WebInputEvent::TouchMove && !processed)
    event_type_can_be_rate_limited = false;
#endif

  bool frame_pending = paint_aggregator_.HasPendingUpdate();
  if (is_accelerated_compositing_active_
// SAMSUNG CHANGE : Patch for spen scrolling is jerky issue
#if defined(S_NATIVE_SUPPORT)
    && input_event->type != WebInputEvent::MouseMove
#endif
  ) {
  ...
  // Do not update input state if the page does complex gesture handling.
  // Otherwise the virtual keyboard would be shown sometimes when it
  // should not.
  if (input_event->type == WebInputEvent::TouchStart)
    seen_processed_touch_move_ = false;
  else if (input_event->type == WebInputEvent::TouchMove && processed)
    seen_processed_touch_move_ = true;
...
void RenderWidget::DoDeferredClose() {
  ...
  // Check for the provision of showing RPP before closing the Tab
  RenderViewImpl* render_view = static_cast<RenderViewImpl*> (this);
  render_view->checkforRPPBeforeTabClose();
  #else
...
#if defined (SBROWSER_DEFERS_LOADING)
void RenderWidget::OnDefersLoading(bool defer) {
  webwidget_->needToDeferLoading(defer);
}
#endif
...
void RenderWidget::UpdateTextInputState(bool show_ime_if_needed,
  ...
  if (show_ime_if_needed || (text_input_type_ != new_type
    ...
    if (webwidget_) {
      p.advanced_ime_options = new_info.advancedIMEOptions;
    } else {
      p.advanced_ime_options = 0;
      new_info.advancedIMEOptions = 0;
    }
#if defined(OS_ANDROID)
...
#if defined(S_JPN_CEDS_0489)
void RenderWidget::UpdateTextInputStateForReset(bool show_ime_if_needed,
                                        bool send_ime_ack) {
  if (handling_ime_event_)
    return;
  if (!show_ime_if_needed && !input_method_is_active_)
...
void RenderWidget::UpdateSelectionBounds(
  ...
  if(isMultiSelection){
    ...
    blink::WebView *webView = static_cast<blink::WebView*>(webwidget_);
    WebRect selection_rect = webView->currentSelectionRect();
    params.selection_rect = gfx::Rect(selection_rect.x, selection_rect.y,
                                      selection_rect.width, selection_rect.height);
    params.is_MultiSel = true;
    #if defined(S_PLM_P140830_01765)
    params.is_image = webwidget_->isAnchorAtImage();
    #endif
