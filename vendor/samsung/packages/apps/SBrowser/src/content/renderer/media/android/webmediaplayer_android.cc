...
static const int kClosedCaptionDisabled = 0;
static const int kClosedCaptionVisible = 1;
static const int kClosedCaptionInvisible = 2;
#endif
...
void WebMediaPlayerAndroid::load(LoadType load_type,
  ...
  if (manager_->ShouldEnterFullscreen(frame_)){
    LOG(INFO) << "[html5media] load. EnterFullscreen because the frame is now in fullscreen mode";
...
#if defined(S_MEDIAPLAYER_FULLSCREEN_CLOSEDCAPTION_SUPPORT)
void WebMediaPlayerAndroid::UpdateCCVisibility(){
    if(!client_->hasClosedCaptions())
        manager_->UpdateCCVisibility(player_id_, kClosedCaptionDisabled);
    else if(client_->closedCaptionsVisible())
        manager_->UpdateCCVisibility(player_id_, kClosedCaptionVisible);
    else
        manager_->UpdateCCVisibility(player_id_, kClosedCaptionInvisible);
}
void WebMediaPlayerAndroid::OnSetCCVisibility(bool visible) {
  client_->setCCVisibility(visible);
}
#endif

#if defined(S_MEDIAPLAYER_POWERSAVERBLOCKER_INVOKEDIDPAUSE)
void WebMediaPlayerAndroid::InvokeDidPause() {
  if (delegate_)
    delegate_->DidPause(this);
}
#endif
...
int WebMediaPlayerAndroid::bufferingPercentage(){
  if (ready_state_ < WebMediaPlayer::ReadyStateHaveMetadata)
    return 0;
  ...
  if (media_source_delegate_){
    buffered_ = media_source_delegate_->Buffered();
    for(unsigned i = 0; i < buffered_.size() ; i++){
        if(current_time_ < buffered_[i].start || current_time_ > buffered_[i].end)
            continue;
        maxBuffered = buffered_[i].end;
    }
  }

  return (int)(maxBuffered * 100 / duration()) > 95 ? 100 : (int)(maxBuffered * 100 / duration());
}
#endif
