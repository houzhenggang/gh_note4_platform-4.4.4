...
const int kConcurrentRasterThreads = 2;
...
#if defined(OS_ANDROID)
  last_view_id_ = MSG_ROUTING_NONE;
#endif
...
#if defined(OS_ANDROID)
  // Disable low resolution tiling by default.
  is_low_res_tiling_enabled_ = false;
  if ((command_line.HasSwitch(switches::kEnableLowResTiling) &&
       !command_line.HasSwitch(switches::kDisableLowResTiling))) {
...
#if defined(OS_ANDROID)
  // Set number of raster threads to 2 for High-end devices.
  else if (!base::android::SysUtils::IsLowEndDevice()) {
    cc::RasterWorkerPool::SetNumRasterThreads(kConcurrentRasterThreads);
  }
#endif
...
void RenderThreadImpl::OnCreateNewView(const ViewMsg_New_Params& params) {
  ...
  LOG(INFO)<<"[Renderer][Startup] RenderThreadImpl::OnCreateNewView " << params.view_id;
#endif
...
#if defined(OS_ANDROID)
  if (MSG_ROUTING_NONE != params.view_id)
    last_view_id_ = params.view_id;
#endif
}
...
#if defined(OS_ANDROID)
int RenderThreadImpl::GetLastViewId() {
  return last_view_id_;
}
#endif
...
void RenderThreadImpl::WidgetRestored() {
  ...
  hidden_widget_count_--;
#if defined(S_UNITTEST_CRASH_FIX)  
