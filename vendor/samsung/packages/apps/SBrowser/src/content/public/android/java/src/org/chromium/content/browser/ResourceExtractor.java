...
public class ResourceExtractor {
    ...
    private static boolean isAllPakExtracted = true;
    ...
    private class ExtractTask extends AsyncTask<Void, Void, Void> {
                        ...
                        try {
                            ...
                            if (os != null) {
    							try {
    								...
    								Log.w(LOGTAG, "Exception during sync(): " + e.getMessage());
    							}
    							finally {
    								try {
    									os.close();
    								} catch (Exception e) {
    									Log.w(LOGTAG, "Exception during close(): " + e.getMessage());
    								}
    							}
    						}
                        }
        ...
        private String checkPakTimestamp() {
            ...
            if (timestamps != null && !expectedTimestamp.equals(timestamps[0])) {
                return expectedTimestamp;
            }
    ...
    public boolean IsPakTimestamp() {
        if (mExtractTask == null) {
            return false;
        }

        return (mExtractTask.checkPakTimestamp() == null);
    }

    public void resetExtractTask() {
        if (mExtractTask != null) {
            mExtractTask = null;
        }
    }
    ...
    public static File getNativeLibraryDirFromContext(Context context) {		
         return new File (context.getApplicationInfo().dataDir, "app_lib");
    }

    public static void deleteFiles(Context context) {
    isAllPakExtracted = false;
    ...
    public boolean isResourceExtractionCompleted(){
    	return isAllPakExtracted;
    }
}
