...
public class AdapterInputConnection extends BaseInputConnection {
    ...
    protected final View mInternalView;
    protected final ImeAdapter mImeAdapter;
    ...
    // JPN IME
    public boolean setComposingTextWithSubComposingRegion(CharSequence text, int newCursorPosition, int newRegionStart, int newRegionEnd, int backgroundColor) {
        if (DEBUG) Log.w(TAG, "AdapterInput setComposingTextWithSubComposing [" + text + "] [" + newCursorPosition + "]");
        super.setComposingText(text, newCursorPosition);
        updateSelectionIfRequired();
        return mImeAdapter.checkCompositionQueueAndCallNativeWithSubComposingRegion(text.toString(),
                newCursorPosition, newRegionStart, newRegionEnd,backgroundColor,  false);
    }
    ...
    public boolean commitText(CharSequence text, int newCursorPosition) {
        ...
        updateSelectionIfRequired();
        return mImeAdapter.checkCompositionQueueAndCallNative(text, newCursorPosition, text.length() > 0);
    }
    ...
    public boolean deleteSurroundingText(int beforeLength, int afterLength) {
        ...
        int availableBefore = Selection.getSelectionStart(getEditable());
        int availableAfter = getEditable().length() - Selection.getSelectionEnd(getEditable());
    ...
    public boolean setComposingRegion(int start, int end) {
        ...
        if (b > a) {
            regionText = getEditable().subSequence(a, b);
        }
