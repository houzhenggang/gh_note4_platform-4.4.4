...
//Samsung changes: This key always mapped to PreferenceKeys 
// fix added for Chromium test cases
    public static final String PREF_RESOURCE_EXTRACTION_COMPLETED = "resource_extraction_completed";
    ...
    public void startBrowserProcessesAsync(final StartupCallback callback)
        ...
        if (!mHasStartedInitializingBrowserProcess) {
            try {
                ...
                Log.i(TAG, "startBrowserProcessesAsync: catch ProcessInitException exception");
                //As resource extraction is failed due to available size, we can terminate browser process here and notify app layer.
                forceCallbackExecution(STARTUP_FAILURE, NOT_ALREADY_STARTED);
                return;
            }
        ...
        if (!mStartupDone) {
            if (!mHasStartedInitializingBrowserProcess) {
                try {
                    ...
                    Log.i(TAG, "startBrowserProcessesSync: catch ProcessInitException exception");
                    forceCallbackExecution(STARTUP_FAILURE, NOT_ALREADY_STARTED);
                    return;
                }
            }
    ...
    private void forceCallbackExecution(final int startupFailure, final boolean alreadyStarted) {
        Log.i(TAG, "forceCallbackExecution: Called");
        executeEnqueuedCallbacks(startupFailure, alreadyStarted);
    }
    ...
    void prepareToStartBrowserProcess(int maxRendererProcesses) throws ProcessInitException {
        ...
        if (!isResourceExtractionCompleted(prefs)) {
         ...
         // Now we really need to have the resources ready.
            ...
            if (resourceExtractor.IsPakTimestamp() == true) {
                 Log.i(TAG, "prepareToStartBrowserProcess: IsPakTimestamp() is TRUE");
            	 Editor editor = prefs.edit();
                 editor.putString(PREF_RESOURCE_EXTRACTION_COMPLETED, LocalizationUtils.getDefaultLocale());
                 editor.commit();
			} else {
                Log.i(TAG, "prepareToStartBrowserProcess: IsPakTimestamp() is FALSE");
				Editor editor = prefs.edit();
                editor.putString(PREF_RESOURCE_EXTRACTION_COMPLETED, null);
                editor.commit();
                resourceExtractor.resetExtractTask();
                throw new ProcessInitException(LoaderErrors.LOADER_ERROR_NATIVE_STARTUP_FAILED);
			}
		}
    ...
    private boolean isResourceExtractionCompleted(SharedPreferences prefs){
        String currentLocale = LocalizationUtils.getDefaultLocale();
        String storedLocale =
                prefs.getString(PREF_RESOURCE_EXTRACTION_COMPLETED,
                        null);
		return (storedLocale!= null && currentLocale.equals(storedLocale));
    }
