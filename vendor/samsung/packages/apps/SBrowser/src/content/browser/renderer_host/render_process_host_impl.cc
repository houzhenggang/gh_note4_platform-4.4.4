...
bool RenderProcessHostImpl::Init() {
  LOG(INFO)<<"[Renderer][Startup] RenderProcessHostImpl::Init";
  ...
  if (run_renderer_in_process()) {
    ...
    LOG(INFO)<<"[Renderer][Startup] RenderProcessHostImpl::Init: Create ChildProcessLauncher";
...
void RenderProcessHostImpl::CreateMessageFilters() {
  ...
  AddFilter(new GamepadBrowserMessageFilter(this));
...
#if defined(ENABLE_PUSH_API)
  AddFilter(new PushRegistrationMessageFilter(GetID(), browser_context->GetPushPermissionContext()));
#endif
}
...
void RenderProcessHostImpl::PropagateBrowserCommandLineToRenderer(
  ...
  static const char* const kSwitchNames[] = {
    ...
    switches::kSoftBitmapScaleFraction,
    ...
    cc::switches::kDisableSbrowserBlockFlingOnEdge,
    ...
    // Doubletap 188 patent DA
    cc::switches::kEnableDoubleTapPatent,
    cc::switches::kIsTablet,
#if defined(ENABLE_PLUGINS)
...
#if !defined(ENABLE_WEBRTC_HW_ACCEL)
    ...
    cc::switches::kDisable4444Textures,
#endif
...
#endif
    switches::kPageLoadSuspendDrawTime
  };
...
bool RenderProcessHostImpl::Send(IPC::Message* msg) {
  ...
  if (!channel_) {
    if (!is_initialized_) {
	  LOG(INFO) << "[RenderProcessHostImpl][Send] channel_ is null and is_initialized_ is null";
      queued_messages_.push(msg);
      return true;
    } else {
	  LOG(INFO) << "[RenderProcessHostImpl][Send] channel_ is null";
      delete msg;
      return false;
    }
  }
  ...
  if (child_process_launcher_.get() && child_process_launcher_->IsStarting()) {
    LOG(INFO) << "[RenderProcessHostImpl][Send] child_process_launcher_ is true";
    queued_messages_.push(msg);
    return true;
  }
...
#if !defined( S_RENDERER_DIED_NOTIFICATION )
...
void RenderProcessHostImpl::ProcessDied(bool already_dead) {
  ...
  LOG(INFO)<<"[Renderer][Exit]	RenderProcessHostImpl::ProcessDied " << GetHandle();
#endif
...
#if defined( S_RENDERER_DIED_NOTIFICATION )
void RenderProcessHostImpl::NotifyRendererDeadWithPid(size_t pid) {
  ProcessDied(true /* already_dead */);
}
#endif //S_RENDERER_DIED_NOTIFICATION
