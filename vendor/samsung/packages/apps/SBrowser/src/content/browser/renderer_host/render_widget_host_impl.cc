...
bool RenderWidgetHostImpl::OnMessageReceived(const IPC::Message &msg) {
  ...
  IPC_BEGIN_MESSAGE_MAP_EX(RenderWidgetHostImpl, msg, msg_is_ok)
    ...
    IPC_MESSAGE_HANDLER(ViewHostMsg_OnCloseSelectPopupZero, OnSelectPopupCloseZero)
    IPC_MESSAGE_HANDLER(ViewHostMsg_Snapshot, OnSnapshot)
	#if defined(S_TRANSPORT_DIB_FOR_SOFT_BITMAP) 
    IPC_MESSAGE_HANDLER(ViewHostMsg_SnapshotDIB, OnSnapshotDIB)
...
void RenderWidgetHostImpl::WasShown() {
  LOG(INFO) << "RenderWidgetHostImpl::WasShown() :: is_hidden_ : "<<is_hidden_ <<"   process_id_ " <<  process_->GetHandle() ;
  if (!is_hidden_)
    return;
  is_hidden_ = false;
...
void RenderWidgetHostImpl::WasResized() {
  ...
  #if defined(SBROWSER_MULTIINSTANCE_TAB_DRAG_AND_DROP) && !defined(S_UNITTEST_SUPPORT)
  if(new_size.IsEmpty() || physical_backing_size_.IsEmpty()) {
    LOG(INFO)<<" RenderWidgetHostImpl- WasResized - new_size w="
      << new_size.width() << " , h =" << new_size.height() << ", w="
      << physical_backing_size_.width() << ", h = " << physical_backing_size_.height();
    if(view_->GetTabDragAndDropIsInProgress() == true) {
      LOG(INFO)<<" WasResized - GetTabDragAndDropIsInProgress = true, hence do not process invalid resize request!!";
      return;
    }
  }
 #endif
...
void RenderWidgetHostImpl::SetHoverHighlight(
    const blink::WebGestureEvent& hover_highlight_event, bool high_light) {
  const blink::WebInputEvent&  hover_event = hover_highlight_event;
  Send(new ViewMsg_HoverHighlight(routing_id_, &hover_event, high_light));
}
...
void RenderWidgetHostImpl::OnSnapshot(bool success,
  ...
  LOG(INFO)<<"RenderWidgetHostImpl::OnSnapshot :: TIMESTAMP :: OnReceived";
...
#if defined(S_SCALING_FOR_SOFT_BITMAP)
  if(device_scale_factor > 2 && soft_bitmap_scale_fraction > 1){
  	SkBitmap bitmap_scaled = ScaleBitmap(bitmap, soft_bitmap_scale_fraction, soft_bitmap_scale_fraction);
  	callback.Run(success, bitmap_scaled);
  }else{
  	LOG(INFO)<<"RenderWidgetHostImpl::OnSnapshot :: soft_bitmap-scale_fraction is one, so not doing any scale up";
...
#if defined(S_SCALING_FOR_SOFT_BITMAP)
SkBitmap RenderWidgetHostImpl::ScaleBitmap(const SkBitmap& src, float scale_x, float scale_y) { 
    LOG(INFO)<<"RenderWidgetHostImpl::ScaleBitmap Called!!!";	
    int width = (int)round(src.width() * scale_x);//Rounding the Width and height, will be an issue when scale factors are fractions 
    int height = (int)round(src.height() * scale_y);//Rounding the Width and height, will be an issue when scale factors are fractions 
    SkBitmap dst; 
    dst.setConfig(src.config(), width, height); //Set the config as the src
    if(!dst.allocPixels()){ 
	LOG(INFO)<<"RenderWidgetHostImpl::ScaleBitmap :: Couldn't allocate pixel buffer for new bitmap, returning the source bitmap, no scaling done";	
    	return src;//if couldnot allocate pixels for new buffer, send the src itself, means no scalling happened.
    }//allocate new PixeRef, size will be adjusted according to config and width, height
    dst.eraseColor(0);//Cleaning up the bitmap, not sure if we have to use eraseARGB(255,255,255,255) 
    SkCanvas canvas(dst); //Create a new canvas, associate with the new bitmap
    canvas.scale(scale_x, scale_y); //Scale the canvas
    canvas.drawBitmap(src, 0, 0, NULL); //draw the old Bitmap to new one from 0,0
    return dst; 
}
#endif//S_SCALING_FOR_SOFT_BITMAP

#if defined(S_TRANSPORT_DIB_FOR_SOFT_BITMAP)
void RenderWidgetHostImpl::OnSnapshotDIB(bool success,
    const gfx::Size& size,
    const TransportDIB::Id& id){
  LOG(INFO)<<"RenderWidgetHostImpl::OnSnapshotDIB :: TIMESTAMP :: OnReceived";
  DCHECK(!size.IsEmpty());
  TransportDIB* dib = process_->GetTransportDIB(id);
  if (!dib) {
    LOG(INFO)<<"RenderWidgetHostImpl::OnSnapshotDIB :: Can't get the DIB";	
  ...
  bitmap_888.setConfig(SkBitmap::kARGB_8888_Config,
      size.width(), size.height());
  bitmap_888.setPixels(dib->memory());
  
  if(!bitmap_888.copyTo(&bitmap_565, SkBitmap::kRGB_565_Config)){
  bitmap_888.setPixels(0);
  LOG(INFO)<<"RenderWidgetHostImpl::OnSnapshotDIB :: Trouble copying to 565 bitmap, returning";
  Send(new ViewMsg_ReleaseSnapshotDIB(GetRoutingID(),
                                                 dib->handle()));
  return;
  }
  ...
  callback.Run(success, bitmap_565);
}
#endif //S_TRANSPORT_DIB_FOR_SOFT_BITMAP 
...
//SBROWSER_FORM_NAVIGATION
void RenderWidgetHostImpl::OnSelectPopupCloseZero() {
#if defined(OS_ANDROID)
  if (view_)
    static_cast<RenderWidgetHostViewAndroid*>(view_)->selectPopupCloseZero();
#endif
}
...
{
#if defined(S_MULTISELECTION_BOUNDS)
  Send(new InputMsg_SelectRange(GetRoutingID(), start, end,isLastTouchPoint,isFirstTouchPoint));
#else
...
void RenderWidgetHostImpl::GetBitmapFromRenderer(
    const gfx::Rect& src_subrect,
    const base::Callback<void(bool, const SkBitmap&)>& callback,
  ...
  pending_snapshots_.push(callback);
  Send(new ViewMsg_CaptureRendererContentSnapShot(
      GetRoutingID(), src_subrect, page_scale_factor));
}

void RenderWidgetHostImpl::FlushPendingCallbacks() {
    if(!pending_snapshots_.empty()) {
	    LOG(INFO)<<" RenderWidgetHostImpl::FlushPendingCallbacks()  pending_snapshots_ is not empty ";
    }
