...
class CONTENT_EXPORT RenderWidgetHostImpl : virtual public RenderWidgetHost,
 ...
 public:
  ...
  virtual void GetSnapshotFromRenderer(
      ...
      const base::Callback<void(bool, const SkBitmap&)>& callback) OVERRIDE;
  virtual void SetHoverHighlight(
      const blink::WebGestureEvent& hover_highlight_event, bool high_light) OVERRIDE;
 ...
 private:
  ...
  /*himangshu.rb :: temp changes for bitmap scaling*/
  SkBitmap ScaleBitmap(const SkBitmap& src, float scale_x, float scale_y);
#endif//S_SCALING_FOR_SOFT_BITMAP
#if defined(S_TRANSPORT_DIB_FOR_SOFT_BITMAP)  
  void OnSnapshotDIB(bool success,
    const gfx::Size& size,
    const TransportDIB::Id& id);
