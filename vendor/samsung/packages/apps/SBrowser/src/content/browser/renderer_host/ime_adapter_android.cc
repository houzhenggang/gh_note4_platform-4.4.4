...
void ImeAdapterAndroid::SetComposingTextWithSubComposingRegion(JNIEnv* env, jobject, jstring text,
                                         int new_cursor_pos, int new_region_start, int new_regsion_end, int backgroundColor) {
  ...
  underlines.push_back(
      blink::WebCompositionUnderline(0, text16.length(), SK_ColorBLACK,
                                      false, new_region_start, new_regsion_end,backgroundColor ));
  // new_cursor_position is as described in the Android API for
  // InputConnection#setComposingText, whereas the parameters for
  // ImeSetComposition are relative to the start of the composition.
  if (new_cursor_pos > 0)
...
void ImeAdapterAndroid::FocusedNodeChanged(bool is_editable_node,bool is_select_node) {
  base::android::ScopedJavaLocalRef<jobject> obj =
      java_ime_adapter_.get(AttachCurrentThread());
  if (!obj.is_null()) {
    Java_ImeAdapter_focusedNodeChanged(AttachCurrentThread(),
                                       obj.obj(),
                                       is_editable_node,is_select_node);
  }
}
...
void ImeAdapterAndroid::DirectPaste(JNIEnv* env, jobject, jstring text) {
  RenderWidgetHost* rwhi = GetRenderWidgetHostImpl();
  if (!rwhi)
    return;

  base::string16 text16 = ConvertJavaStringToUTF16(env, text);
  rwhi->Send(new InputMsg_DirectPaste(rwhi->GetRoutingID(), text16));
}
...
void ImeAdapterAndroid::DefersLoading(JNIEnv* env, jobject, jboolean defer) {
  #if defined (SBROWSER_DEFERS_LOADING)
  RenderWidgetHostImpl* rwhi = GetRenderWidgetHostImpl();
  if (!rwhi)
    return;
  rwhi->Send(new ViewMsg_DefersLoading(rwhi->GetRoutingID(), defer));
  #endif
}

void ImeAdapterAndroid::RequestUpdateIME(JNIEnv* env, jobject, jboolean show_ime_if_needed, jboolean send_ime_ack ) {    
    ...
    if (!rwhi)
      return;
  rwhi->Send(new ViewMsg_UpdateTextInputState(rwhi->GetRoutingID(), show_ime_if_needed,send_ime_ack ));  
#endif
}
