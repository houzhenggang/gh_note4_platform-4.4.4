...
//Notifying browser process about renderer died
#if defined( S_RENDERER_DIED_NOTIFICATION )

static void OnChildProcessDied(JNIEnv*,jclass,jint pid)
{
  int renderer_id = 0;
  RenderProcessHost::iterator it = RenderProcessHost::AllHostsIterator();
  while (!it.IsAtEnd()) {
  ...
  LOG(INFO)<<"[Renderer][Exit] ChildProcessLauncher::OnChildProcessDied called through JNI pid = " <<pid <<", renderer_id ="<< renderer_id;
  if (renderer_id) {
    RenderProcessHost* process_host  = RenderProcessHost::FromID(renderer_id);
    LOG(INFO)<<"[Renderer][Exit] ChildProcessLauncher::OnChildProcessDied called through JNI  process_host = " << process_host;
    if (process_host) {
      process_host->NotifyRendererDeadWithPid(pid);
    }
  }
}
