...
RenderProcessHost* SiteInstanceImpl::GetProcess() {
  ...
  if (!process_) {
    LOG(INFO)<<"[Renderer][Startup] SiteInstanceImpl::GetProcess: process_ is 0. So getting new";
    ...
    if (!process_ && RenderProcessHostImpl::ShouldTryToUseExistingProcessHost(
      ...
      LOG(INFO)<<"[Renderer][Startup] SiteInstanceImpl::GetProcess GetExistingProcessHost";
    ...
    if (!process_) {
      if (g_render_process_host_factory_) {
        ...
        LOG(INFO)<<"[Renderer][Startup] SiteInstanceImpl::GetProcess: Creating New RenderProcessHostImpl";
...
SiteInstance* SiteInstanceImpl::GetRelatedSiteInstanceWithoutURL() {
  return browsing_instance_->GetSiteInstanceWithoutURL();
}
#endif
