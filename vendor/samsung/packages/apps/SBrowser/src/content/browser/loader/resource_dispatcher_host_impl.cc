...
#define RDH_PENDING_REQUEST_COUNT 100
// SAMSUNG CHANGE
#endif
...
#if !defined(S_UNITTEST_SUPPORT)
// SAMSUNG CHANGE: logging pending request(timeout: 10s)
  loadstate_update_count_ = 0;
  kTenSecond_ = TimeDelta::FromMilliseconds(10000);
// SAMSUNG CHANGE
#endif
...
void ResourceDispatcherHostImpl::OnRequestResource(
  ...
  if(request_data.resource_type == ResourceType::MAIN_FRAME)
    LOG(INFO) << "ResourceDispatcherHostImpl::OnRequestResource mainframe request_id=" << request_id;
...
#if !defined(S_UNITTEST_SUPPORT)
// SAMSUNG CHANGE: logging pending request(timeout: 10s)
  LoadInfoMap delaying_loader_info_map;
  loadstate_update_count_++;
// SAMSUNG CHANGE
#endif
...
#if !defined(S_UNITTEST_SUPPORT)
// SAMSUNG CHANGE: logging pending request(timeout: 10s)
    if(loadstate_update_count_ >= RDH_PENDING_REQUEST_COUNT && TimeTicks::Now() - i->second->request_start_time() > kTenSecond_){
      LoadInfo& delaying_load_info = delaying_loader_info_map[id];
      delaying_load_info.url = request->url();
      loadstate_update_count_ = 0;
    }
//SAMSUNG CHANGE
#endif
...
#if !defined(S_UNITTEST_SUPPORT)
// SAMSUNG CHANGE: logging pending request(timeout: 10s)
  if(!delaying_loader_info_map.empty()){
    LoadInfoMap::const_iterator i;
    for (i = info_map.begin(); i != info_map.end(); ++i) {
      std::string encodeStr;
      base::Base64Encode(i->second.url.spec(), &encodeStr);
      VLOG(0) << "Pending Request:" << encodeStr;
    }
  }
// SAMSUNG CHANGE
#endif
