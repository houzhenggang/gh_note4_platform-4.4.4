...
#include "out_jni/ContentViewCore_jni.h"
...
ContentViewCoreImpl::~ContentViewCoreImpl() {
  LOG(INFO) << __FUNCTION__ <<"Inside destructor" << this;
  ...
  LOG(INFO) << __FUNCTION__ << "RemoveAll done";
}
...
void ContentViewCoreImpl::OnTabCrashed() {
  LOG(INFO) << "TabCrashCheck" <<" OnTabCrashed";
  JNIEnv* env = AttachCurrentThread();
  ScopedJavaLocalRef<jobject> obj = java_ref_.get(env);
  if (obj.is_null())
    return;
  ...
  java_bridge->NotifyTabCrash(render_view_host);
...
void ContentViewCoreImpl::Reload(JNIEnv* env,
  ...
  if (web_contents_->GetController().NeedsReload()) {
    LOG(INFO)<<"[Renderer][Startup] ContentViewCoreImpl::Reload - LoadIfNecessary";
    web_contents_->GetController().LoadIfNecessary();
    }
  else {
    LOG(INFO)<<"[Renderer][Startup] ContentViewCoreImpl::Reload - Reload";
...
void ContentViewCoreImpl::OnSmartClipDataExtracted(
  ...
  ScopedJavaLocalRef<jstring> jresult = ConvertUTF16ToJavaString(env, result);
   ScopedJavaLocalRef<jstring> jresulthtml = ConvertUTF16ToJavaString(env, innerHtml);
  Java_ContentViewCore_onSmartClipDataExtracted(
      env, obj.obj(), jresult.obj(),jresulthtml.obj());
}
