...
class RenderWidgetHostViewAndroid
 ...
 public:
  ...
  virtual void FocusedNodeChanged(bool is_editable_node,bool is_select_node,
                                  long node_id) OVERRIDE;
  ...
  //Based on how often JIT is triggered the V8 engine will notify
  //browser V8 is really busy/hot
  virtual void OnSSRMModeCallback(int SSRMCaller, int count) OVERRIDE;
  ...
  void OnSmartClipDataExtracted(const base::string16& result, const base::string16& innerHTML);
  void OnUpdateFocusedInputInfo(const gfx::Rect& bounds, bool is_multi_line_editable, bool is_content_richly_editable);
  ...
  void CopyFromCompositingSurfaceToSkBitmap(
    const gfx::Rect& src_subrect,
    const gfx::Size& dst_size,
    const base::Callback<void(bool, const SkBitmap&)>& callback,
  ...
  void SelectRange(const gfx::Point& start, const gfx::Point& end
		...
		,bool isLastTouchPoint=false,bool isFirstTouchPoint=false
  #endif
  );

  void GetSelectionVisibilityStatus();
  void CheckBelongToSelection(int x, int y);
  void GetSelectionBitmap();
  void SelectClosestWord(int x, int y);
  void ClearTextSelection();

  void SelectLinkText(const gfx::Point&);
  void GetTouchedFixedElementHeight(int x, int y);

  void GetBitmapFromCachedResource(const std::string& image_url);
  ...
  void SetTopControlsHeight(int top_controls_height);
#endif
...
// MULTI-SELECTION >>
  ...
  void OnSelectedMarkupWithStartContentRect (
...
#if defined(SBROWSER_HIDE_URLBAR_HYBRID)  
  // The most recent content offset that was pushed to the texture layer.
