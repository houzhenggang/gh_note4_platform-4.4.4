...
#if !defined(S_UNITTEST_SUPPORT)
// SAMSUNG CHANGE: idle connection closing immediately after loading finished.
void NotifyLoadStarted(scoped_refptr<net::URLRequestContextGetter> request_context){
  request_context->GetURLRequestContext()->set_main_frame_load_finished(false);
}

void NotifyLoadFinished(scoped_refptr<net::URLRequestContextGetter> request_context){
  net::HttpNetworkSession* session = request_context->GetURLRequestContext()->http_transaction_factory()->GetSession();
  if (session)
    session ->CloseIdleConnections();

  request_context->GetURLRequestContext()->set_main_frame_load_finished(true);
}
// SAMSUNG CHANAGE
#endif
...
WebContentsImpl::~WebContentsImpl() {
  ...
  LOG(INFO)<<"WebContentsImpl::~WebContentsImpl Destructor";
...
WebContentsImpl* WebContentsImpl::CreateWithOpener(
  ...
  new_contents->Init(params);
  LOG(INFO) << __FUNCTION__ << " new web contents = " << new_contents;
  return new_contents;
}
...
void WebContentsImpl::SavePageAs(
    const base::FilePath::StringType& pure_file_name) {
  if (GetRenderViewHost())
    GetRenderViewHostImpl()->SavePageAs(pure_file_name);
}
...
// SAMSUNG CHANGE: idle connection closing immediately after loading finished.
  if(is_main_frame){
    RenderProcessHost* renderProcessHost = GetRenderProcessHost();
    if(renderProcessHost != NULL){
	  scoped_refptr<net::URLRequestContextGetter> request_context(GetBrowserContext()->GetRequestContextForRenderProcess(renderProcessHost->GetID()));
	  if(request_context.get() != NULL)
	    BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,base::Bind(&NotifyLoadFinished, request_context));
    }
  }
// SAMSUNG CHANGE
#endif
...
// SAMSUNG CHANGE: idle connection closing immediately after loading finished.
  ...
  if(renderProcessHost != NULL){
    scoped_refptr<net::URLRequestContextGetter> request_context(GetBrowserContext()->GetRequestContextForRenderProcess(renderProcessHost->GetID()));
    if(request_context.get() != NULL)
      BrowserThread::PostTask(BrowserThread::IO, FROM_HERE,base::Bind(&NotifyLoadStarted, request_context));
  }
// SAMSUNG CHANGE: idle connection closing immediately after loading finished.
#endif
