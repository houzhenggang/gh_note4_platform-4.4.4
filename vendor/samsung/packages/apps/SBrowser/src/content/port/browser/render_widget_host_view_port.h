...
class CONTENT_EXPORT RenderWidgetHostViewPort : public RenderWidgetHostView,
 ...
 public:
  ...
  virtual void FocusedNodeChanged(bool is_editable_node, bool is_select_node,long node_id) = 0;
  ...
  virtual bool GetTabDragAndDropIsInProgress() const { return false;}
  #endif
  ...
  virtual void OnRecognizeArticleResult(std::string reader_result_str) { };
  ...
  //callback To detect v8 is hot based on how often JIT is triggered
  virtual void OnSSRMModeCallback(int SSRMCaller, int count){}; //0 is v8, 1 is css, 2 is canvas, 3 is etc
#endif
