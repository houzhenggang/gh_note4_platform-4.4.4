...
public class SbrContentVideoViewNew extends SbrContentVideoView implements SbrVideoControllerView.MediaPlayerControl{
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        ...
        if(surfaceView != null && surfaceView.isPresentationMode()){
            Log.d(TAG,"[html5media] surfaceDestroyed returns.");
            return;
        }
        ...
        surfaceView.setOnKeyListener(new OnKeyListener() {
            ...
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                ...
                if (isInPlaybackState() && isKeyCodeSupported && mMediaController != null) {
                    ...
                    exitFullscreen(false);
                    return true;
                } else if (keyCode == KeyEvent.KEYCODE_VOLUME_UP && event.getAction() == KeyEvent.ACTION_DOWN && isPlaying()) {
                    if (!mIsVideoGestureStart && mMediaController != null && mMediaController.keyVolumeUp()) {
                        mMediaController.showPopupVolbar();
                    }
                    return true;
                } else if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN && event.getAction() == KeyEvent.ACTION_DOWN && isPlaying()) {
    ...
    public void exitFullscreen(boolean relaseMediaPlayer) {
        ...
        if(mWFDManager != null){
            mWFDManager.dismissPresentation();
