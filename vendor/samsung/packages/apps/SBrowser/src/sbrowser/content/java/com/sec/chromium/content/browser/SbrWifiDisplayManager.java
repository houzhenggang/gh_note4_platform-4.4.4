...
public class SbrWifiDisplayManager{
    ...
    private void makeWfdDeviceList() {
        ...
        while (iter.hasNext()) {
            ...
            if(wfdName.equals(mConnectedDeviceName)) continue;
    ...
    private void selectWfdDevice(int which) {
        ...
        Log.d(TAG, "[html5media] selectWfdDevice. connectingDeviceName : " + connectingDeviceName);

        // selecting currently connected device
        if(isWfdConnected()){
            if(mConnectedDeviceName.equals(connectingDeviceName)){
                Log.d(TAG, "[html5media] selectWfdDevice. selected the same device");
                mConnecting = false;
                return;
            }
        }
        ...
        if(mPlayerList.get(which).isMyDevice()){
            Log.d(TAG, "[html5media] selected MyDevice");

            // return if currently my device is selected
            if(mConnectedDeviceName == null
                || mConnectedDeviceName.equals(mContext.getString(R.string.my_device))
                || !isPresentationStarted())
                return;
            
            SbrContentVideoViewNew videoView = (SbrContentVideoViewNew)SbrContentVideoViewNew.getSbrContentVideoView();
            videoView.pause();
            
            stopScanWifiDisplays();
            dismissPresentation();
            videoView.showContentVideoView();
    ...
    public void updateWifiDisplayList() {
        ...
        if(mDialog != null && mDialog.isShowing() && mPlayerList != null && mChangePlayerAdapter != null) {
            ((SBrowserMainActivity)mContext).runOnUiThread(new Runnable() {
                public void run() {
                    Log.d(TAG,"[html5media] updateWifiDisplayList. notifyDataSetChanged");
    ...
    private boolean checkSideSyncConnected() {
        if (mContext == null || mContext.getContentResolver() == null) return false;
    ...
    private void showPresentation(Display display) {
        Log.d(TAG,"[html5media] showPresentation. " + display.getName());
    ...
    public void showPresentationAutoplay(boolean wasPlaying) {
        Log.d(TAG,"[html5media] showPresentationAutoplay. " + isWfdConnected());
        
        if (!isWfdConnected() || isPresentationStarted() || checkSideSyncConnected()) {
            Log.d(TAG,"[html5media] showPresentationAutoplay RETURN.");
            return;
        }
    ...
    public void dismissPresentation(){
        Log.d(TAG,"[html5media] dismiss");
        if(mMainView != null && mPresentationView != null){
            Log.d(TAG,"[html5media[ removing views");
    ...
    private final class VideoPresentation extends Presentation {
        ...
        @Override public void onDisplayChanged (){
            Log.d(TAG,"[html5media] VideoPresentation. onDisplayChanged");
            return;
        }
