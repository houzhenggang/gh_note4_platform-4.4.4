...
public class ContentViewCore
    ...
    public interface SmartClipDataListener {
        public void onSmartClipDataExtracted(String result, String innerHTML);
    }
    ...
    @CalledByNative
    public void addVSyncSubscriber() {
    ...
    @CalledByNative
    protected void resetVSyncNotification() {
    ...
    protected boolean isVSyncNotificationEnabled() {
    ...
    public void onStartHoverScroll() {
        return;
    }
    ...
    protected WebContentsObserverAndroid mWebContentsObserver;

    private ContentViewClient mContentViewClient;

    protected ContentSettings mContentSettings;

    // Native pointer to C++ ContentViewCoreImpl object which will be set by nativeInit().
    protected long mNativeContentViewCore = 0;

    private boolean mInForeground = false;

    protected ContentViewGestureHandler mContentViewGestureHandler;
    ...
    protected SelectPopupDialog mSelectPopUpDialog;

    protected Runnable mFakeMouseMoveRunnable = null;
    ...
    protected SelectionHandleController mSelectionHandleController;
    protected InsertionHandleController mInsertionHandleController;

    private Runnable mDeferredHandleFadeInRunnable;

    protected PositionObserver mPositionObserver;
    protected PositionObserver.Listener mPositionListener;
    ...
    protected final RenderCoordinates.NormalizedPoint mStartHandlePoint;
    protected final RenderCoordinates.NormalizedPoint mEndHandlePoint;
    protected final RenderCoordinates.NormalizedPoint mInsertionHandlePoint;

    // Tracks whether a selection is currently active.  When applied to selected text, indicates
    // whether the last selected text is still highlighted.
    protected boolean mHasSelection;
    protected String mLastSelectedText;
    protected boolean mSelectionEditable;
    private ActionMode mActionMode;
    protected boolean mUnselectAllOnActionModeDismiss;
    ...
    protected AccessibilityInjector mAccessibilityInjector;
    ...
    protected BrowserAccessibilityManager mBrowserAccessibilityManager;
    ...
    protected boolean mUnfocusOnNextSizeChanged = false;
    protected final Rect mFocusPreOSKViewportRect = new Rect();
    ...
    protected ViewAndroid mViewAndroid;
    ...
    public ContentViewCore(Context context) {
        ...
        mAdapterInputConnectionFactory = initAdapterInputConnectionFactory();
    ...
    protected AdapterInputConnectionFactory initAdapterInputConnectionFactory() {
        return new AdapterInputConnectionFactory();
    }
    ...
    protected ImeAdapter createImeAdapter(Context context) {
    ...
    public void initialize(ViewGroup containerView, InternalAccessDelegate internalDispatcher,
        ...
        if (windowNativePointer != 0) {
            if (mViewAndroid == null) mViewAndroid = new ViewAndroid(windowAndroid, getViewAndroidDelegate());
        ...
        if (mContentViewGestureHandler == null) mContentViewGestureHandler = new ContentViewGestureHandler(mContext, this);
        ...
        if (mNativeContentViewCore == 0) mNativeContentViewCore = nativeInit(
        ...
        if (com.sec.android.app.sbrowser.R.string.accessibility_content_view == 0) {
            ...
            contentDescription = mContext.getResources().getString(
                    com.sec.android.app.sbrowser.R.string.accessibility_content_view);
        }
    ...
    void onNativeContentViewCoreDestroyed(long nativeContentViewCore) {
        ...
        Log.d(TAG, "APPLOGS:onNativeContentViewCoreDestroyed " + mNativeContentViewCore);
    }
    ...
    protected void initPopupZoomer(Context context) {
        if (mPopupZoomer == null)
     ...
     */
    public void destroy() {
        Log.i(TAG,"ContentViewCore.destroy begin " + this);
        if (mNativeContentViewCore != 0) {
        ...
        Log.i(TAG,"ContentViewCore.destroy end");
    }
     ...
     */
    public void onShow() {
        Log.i(TAG,"ContentViewCore.onShow()");
     ...
     */
    public void onHide() {
        Log.i(TAG,"ContentViewCore.onHide()");
        assert mNativeContentViewCore != 0;
        if (mInForeground) {
            int pid = nativeGetCurrentRenderProcessId(mNativeContentViewCore);
    ...
    protected void onRenderCoordinatesUpdated() {
    ...
    private void hidePopupDialog() {
        if (mSelectPopUpDialog != null)
            mSelectPopUpDialog.hide(this);
        hideHandles();
        hideSelectActionBar();
    }

    protected void hideSelectActionBar() {
        if (mActionMode != null) {
            mActionMode.finish();
            mActionMode = null;
        }
    ...
    protected void scrollFocusedEditableNodeIntoView() {
        if (mNativeContentViewCore != 0) {
    ...
    protected void undoScrollFocusedEditableNodeIntoViewIfNeeded(boolean backButtonPressed) {
    ...
    protected void updateForTapOrPress(int type, float xPix, float yPix) {
     ...
     * FIXME(sl): Obsolette API for sbrowser comaptibilty. Fix sbrowser and remove.
     */
    protected void sendOrientationChangeEvent() {
        if (mNativeContentViewCore == 0) return;
    ...
    protected void updateHandleScreenPositions() {
            ...
            public void share() {
                ...
                try {
                    Intent i = Intent.createChooser(send, getContext().getString(
                            com.sec.android.app.sbrowser.R.string.actionbar_share));
    ...
    protected boolean isSelectionHandleShowing() {
        return mSelectionHandleController != null && mSelectionHandleController.isShowing();
    }

    protected boolean isInsertionHandleShowing() {
    ...
    protected void temporarilyHideTextHandles() {
    ...
    public void setPredictionScroll( boolean value ) {
        mContentViewGestureHandler.setPredictionScroll( value );
    }

    // Cancels any pending fade in and schedules a new one.
    protected void scheduleTextHandleFadeIn() {
    ...
    @CalledByNative
    protected void updateImeAdapter(int nativeImeAdapterAndroid, int textInputType,
    ...
    private void showSelectPopup(String[] items, int[] enabled, boolean multiple,
        ...
        if (mSelectPopUpDialog != null) {
            mSelectPopUpDialog.show(this, popupItems, multiple, selectedIndices);
        }
    }

    @SuppressWarnings("unused")
    @CalledByNative
    protected void showDisambiguationPopup(Rect targetRect, Bitmap zoomedBitmap) {
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected TouchEventSynthesizer createTouchEventSynthesizer() {
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected void onSelectionBoundsChanged(Rect anchorRectDip, int anchorDir, Rect focusRectDip,
    ...
    @SuppressWarnings("unused")
    @CalledByNative
    protected void onRenderProcessSwap(int oldPid, int newPid) {
        Log.d(TAG, "ContentViewCore::onRenderProcessSwap() oldPid = " + oldPid + ", newPid = " + newPid);
        if (!mInForeground) {
    ...
    private void onWebContentsConnected() {
        Log.d(TAG, "ContentViewCore::onWebContentsConnected()");
        attachImeAdapter();
    }
    ...
    public void updateTopControlsState(boolean enableHiding, boolean enableShowing,
	    ...
	    // SMCL_POWER [
       	    // For analyzing purpose only for some time 
       	    Log.d(TAG,"updateTopControlsState enableHiding="+enableHiding+ " enableShowing="+enableShowing);
       	    // SMCL_POWER ]
    ...
    private void addToNavigationHistory(Object history, int index, String url, String virtualUrl,
        ...
        NavigationEntry entry = new NavigationEntry(
                index, url, virtualUrl, originalUrl, title, /*favicon*/null);
    ...
    @CalledByNative
    private void onSmartClipDataExtracted(String result, String innerHtml) {
        if (mSmartClipDataListener != null ) {
            mSmartClipDataListener.onSmartClipDataExtracted(result, innerHtml);
        }
    }
    ...
    public void stopChildProcessOnRendererUnresponsive(int pid) {
        Log.e(TAG, "[stopChildProcessOnRendererUnresponsive] PID = " + pid);
    ...
    protected native void nativeSetFocus(long nativeContentViewCoreImpl, boolean focused);
    ...
    protected native int nativeSendMouseMoveEvent(
    ...
    protected native void nativeScrollBegin(
            long nativeContentViewCoreImpl, long timeMs, float x, float y, float hintX,
            float hintY);

    protected native void nativeScrollEnd(long nativeContentViewCoreImpl, long timeMs);
    ...
    protected native void nativeFlingCancel(long nativeContentViewCoreImpl, long timeMs);

    protected native void nativeSingleTap(
    ...
    protected native void nativeLongPress(
            long nativeContentViewCoreImpl, long timeMs, float x, float y, boolean linkPreviewTap);
    ...
    protected native void nativeSelectBetweenCoordinates(
            long nativeContentViewCoreImpl, float x1, float y1, float x2, float y2);

    protected native void nativeMoveCaret(long nativeContentViewCoreImpl, float x, float y);
    ...
    protected native int nativeGetNativeImeAdapter(long nativeContentViewCoreImpl);
    ...
    protected native void nativeOnHide(long nativeContentViewCoreImpl);
