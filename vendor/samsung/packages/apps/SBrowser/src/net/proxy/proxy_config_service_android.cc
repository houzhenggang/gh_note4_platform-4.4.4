...
class ProxyConfigServiceAndroid::Delegate
 ...
 public:
  ...
  // Called on the JNI thread.
#if defined(S_PROXY_BYPASS_RULES)
  void ProxySettingsChangedTo(const std::string& host, int port, const std::string& bypass) {
    DCHECK(OnJNIThread());
    ProxyConfig proxy_config;
    CreateStaticProxyConfig(host, port, &proxy_config, bypass);
    network_task_runner_->PostTask(
        FROM_HERE,
        base::Bind(
            &Delegate::SetNewConfigOnNetworkThread, this, proxy_config));
  ...
  class JNIDelegateImpl : public ProxyConfigServiceAndroid::JNIDelegate {
   public:
    ...
    virtual void ProxySettingsChangedTo(JNIEnv* env, jobject jself,
                                      ...
                                      jstring jbypass) OVERRIDE {
      std::string host = ConvertJavaStringToUTF8(env, jhost);
#if defined(S_PROXY_BYPASS_RULES)
      std::string bypass = ConvertJavaStringToUTF8(env, jbypass);
      delegate_->ProxySettingsChangedTo(host, jport, bypass);
#else
