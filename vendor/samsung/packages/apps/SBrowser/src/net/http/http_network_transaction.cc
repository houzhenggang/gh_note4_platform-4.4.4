...
#if defined(S_HTTP_REQUEST_HEADERS)
  if (!HttpRequestHeaders::getSbrExtraHeaders().IsEmpty())
    request_headers_.MergeFrom(HttpRequestHeaders::getSbrExtraHeaders());
  // SAMSUNG CHANGE : remove x-agent-profile header if user-agent doesn't have "Android".
  std::string value;
  request_headers_.GetHeader(HttpRequestHeaders::kUserAgent, &value);
  if (value.find("Android") == std::string::npos)
  	request_headers_.RemoveHeader("x-wap-profile");
#endif
...
#if defined(S_SAMSUNG_CLIENT_CERT_POLICY)
  // remove ssl client auth cache, if the server gives 403 forbidden error
  // Microsoft-IIS gives 403 forbidden instead of encrypted alert,
  // when it receives the invalid certificate
  // http://support.microsoft.com/kb/186812
  if (server_ssl_config_.send_client_cert &&
      response_.headers->response_code() == HTTP_FORBIDDEN) {
...
int HttpNetworkTransaction::HandleAuthChallenge() {
  ...
  int rv = auth_controllers_[target]->HandleAuthChallenge(
      ...
      response_.did_use_http_auth ,net_log_);
#else
  int rv = auth_controllers_[target]->HandleAuthChallenge(
      headers, (request_->load_flags & LOAD_DO_NOT_SEND_AUTH_DATA) != 0, false,
      net_log_);
