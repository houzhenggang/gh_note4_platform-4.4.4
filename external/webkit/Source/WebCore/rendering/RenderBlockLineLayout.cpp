...
void RenderBlock::layoutInlineChildren(bool relayoutChildren, int& repaintLogicalTop, int& repaintLogicalBottom)
{
    ...
    if (firstChild()) {
        ...
        while (!end.atEnd()) {
            ...
            if(settings && settings->isSkipLayoutForHiddenHorizontalText()) {
               if (style() && style()->height().value()) {
                    if (style()->overflowY() == OHIDDEN 
                        ...
                        && logicalHeight() > style()->height().value())                
                        break;
                }  
            }
        }
...
static bool isEmojiUnicode(int c) {
    if(c == 127344 || c == 127345 || c == 127358 || c == 127359 || ( c >= 127374 && c <= 127385) 
        || (c >= 128306 && c <= 128317) ||  (c >= 128160 && c <= 128164) || c == 128175 
        || (c >= 128256 && c <= 128262) || (c >= 128279 && c <= 128292) )
        return true;
    else
        return false;
}
        ...
        if (o->isFloatingOrPositioned()) {
            ...
            while (len) {
                ...
                if (c >= 0x3041 && len > 1) {
                    ...
                    UChar c2 = str[pos+1];
                    int  c3 = (c << 10) + c2 + (0x10000 - (0xD800 << 10) - 0xDC00);
                    if(isExtendedUnicode = isEmojiUnicode(c3))
                        extendedCharCount = 1;
                }
                // SAMSUNG CHANGE <<
